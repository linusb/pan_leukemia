...
path <- "../../predictions/"
out_path <- "complete_substraction/"
...
# all to upper
benign_peptides <-sapply(benign_peptides, toupper)
...
##################################
# Substract all benign peptides  #
##################################
specific_binders <- binders[which(!binders$Seq %in% benign_peptides),]


# write statistic tables
statistic_table <- data.frame(matrix(ncol= length(unique(specific_binders$Patient)), nrow = length(alleles)))
row.names(statistic_table) <- alleles
colnames(statistic_table) <- ((unique(specific_binders$Patient)))
for(p in colnames(statistic_table)){
  patient_binders <- specific_binders[which(specific_binders$Patient == p),]
  for(a in alleles){
    statistic_table[a,p] <- nrow(patient_binders[which(patient_binders[,paste0(a,"_aff")]<=500 | patient_binders[,paste0(a,"_rank")]<=2 ),])
  }
}
write.table(statistic_table, file = paste0(out_path,"number_of_binders_per_patient_tumor_specific.csv"), sep = ';', dec = ",", row.names = T, col.names = NA)
...
#peptide_protein_list <- protein_mapper(specific_binders, "swissprotHUMANwoi_130927.fasta")
...
# row.names(statistic_table) <- alleles
# colnames(statistic_table) <- all_patients
# for(p in colnames(specific_binders)){
#   patient_binders <- specific_binders[which(specific_binders$Patient == p),]
#   for(a in alleles){
#     statistic_table[a,p] <- nrow(patient_binders[which(patient_binders[,paste0(a,"_aff")]<=500 | patient_binders[,paste0(a,"_rank")]<=2 ),])
#   }
...
for(allele in alleles){
  ...
  tumor_specific_allotype_binder_table <- allotype_binder_table_calculator(binders = specific_binders, allele = allele)
  ...
  tumor_specific_allotype_binder_table <- tumor_specific_allotype_binder_table
  ...
  ###########################
  # get all allotype typings
  allotype_binder_table <- allotype_binder_table_calculator(specific_binders, allele)
  
  tumor_allotype_binder_table <- allotype_binder_table[,which(colnames(allotype_binder_table) %in% tumor)]
  #benign_allotype_binder_table <- allotype_binder_table[,which(colnames(allotype_binder_table) %in% benign)]
  # tumor specific
  tumor_specific_allotype_binder_table <- tumor_allotype_binder_table
  ...
  # only peptides with one protein   #
  ####################################
  allotype_binder_table <- allotype_binder_table_calculator(binders = specific_binders, allele = allele)
